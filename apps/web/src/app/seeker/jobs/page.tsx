...
export default async function SeekerJobsPage({ searchParams }: { searchParams: Promise<{ q?: string; location?: string; view?: string }> }) {
  ...
  return <>
    ...
    <SectionCard title="Filters" description="The full mobile filter set, arranged for desktop." action={<Link className="text-button" href="/seeker/jobs">Clear all</Link>}>
      ...
      <div className="filter-footer"><span><SlidersHorizontal size={16} /> Also supports nationality, designation, gender preference, and salary currency.</span><PreviewActionButton type="button" className="filter-button" storageKey="seeker-jobs-save-search" successLabel="Search saved"><Bell size={16} /> Save search</PreviewActionButton></div>
    </SectionCard>
